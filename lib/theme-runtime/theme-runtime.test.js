...
describe('buildCartBridge.addItem — field translation', () => {
  ...
  // Only a REAL variant may reach the cart: its id goes to checkout as variantId,
  // and a made-up one ("XL" on a product with no variants) failed the order.
  function captureVariant(product, sizeOrVariant) {
    let captured = 'not-called'
    const actions = {
      addToCart: (p, qty, v) => { captured = v },
      removeFromCart: () => {}, changeQty: () => {}, setCartOpen: () => {},
    }
    const cart = buildCartBridge({ state: { cart: [], cartOpen: false, cartTotal: 0, cartCount: 0 }, actions, slug: 'test-store' })
    cart.addItem(product, 1, sizeOrVariant)
    return captured
  }
  const variants = [
    { id: 'v1', name: 'M', price: null, stock: 5, sku: '' },
    { id: 'v2', name: 'Ash / L', price: 900, stock: 2, sku: '' },
  ]

  test('product without variants: a size string adds the product with no variant', () => {
    assert.equal(captureVariant({ id: 'p1', name: 'Shirt', price: 500, image: null, variants: [] }, 'XL'), null)
    assert.equal(captureVariant({ id: 'p1', name: 'Shirt', price: 500, image: null }, 'M'), null)
  })

  test('size string resolves to the real variant by name (case/space-insensitive)', () => {
    assert.equal(captureVariant({ id: 'p1', price: 500, variants }, 'M').id, 'v1')
    assert.equal(captureVariant({ id: 'p1', price: 500, variants }, ' ash / l ').id, 'v2')
  })

  test('variant id string or object resolves to the real variant', () => {
    assert.equal(captureVariant({ id: 'p1', price: 500, variants }, 'v2').id, 'v2')
    assert.deepEqual(captureVariant({ id: 'p1', price: 500, variants }, { id: 'v1', name: 'M' }), variants[0])
  })

  test('no size, or a size that matches nothing, adds no variant', () => {
    assert.equal(captureVariant({ id: 'p1', price: 500, variants }, null), null)
    assert.equal(captureVariant({ id: 'p1', price: 500, variants }, 'XXL'), null)
  })
})
