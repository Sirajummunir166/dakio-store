...
/**
 * Resolve what a theme passed as the size/variant to a REAL variant of the product.
 * Themes pass a size string (often the variant's name, sometimes a made-up default
 * like "M") or a variant object. Only a real variant may reach the cart: its id is
 * sent to checkout as variantId, and a fake one fails the order.
 * Returns the variant, or null when the product has no variants / nothing matches.
 */
export function resolveVariant(product, sizeOrVariant) {
  const variants = Array.isArray(product?.variants) ? product.variants : []
  if (variants.length === 0 || sizeOrVariant == null || sizeOrVariant === '') return null
  const wanted = typeof sizeOrVariant === 'object' ? sizeOrVariant : { id: sizeOrVariant, name: sizeOrVariant }
  const norm = (s) => String(s ?? '').trim().toLowerCase()
  return (
    variants.find((v) => wanted.id && v.id === wanted.id) ||
    variants.find((v) => wanted.name && norm(v.name) === norm(wanted.name)) ||
    null
  )
}
...
export function buildCartBridge({ state, actions, slug }) {
  return {
    ...
    addItem: (normalizedProduct, qty, sizeOrVariant) => {
      ...
      actions.addToCart(forCart, qty, resolveVariant(normalizedProduct, sizeOrVariant))
    },
