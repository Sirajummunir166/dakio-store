...
export function useStorefront({ store: initialStore, products: initialProducts, categories: initialCategories, slug }) {
  ...
  function setDetail(product) {
    ...
    if (product.slug) {
      const isCustomDomain = typeof window !== 'undefined' && !window.location.pathname.startsWith(`/${slug}`)
      router.push(isCustomDomain ? `/products/${product.slug}` : `/${slug}/products/${product.slug}`)
      return
    }
  ...
  function tryLeadCapture() {
    ...
    // Set a sessionStorage flag so CheckoutClient won't double-fire browser Lead
    // if the user navigates from this page to the checkout page in the same session.
