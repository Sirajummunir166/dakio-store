...
export function useStorefront({ store: initialStore, products: initialProducts, categories: initialCategories, slug }) {
  ...
  function addToCart(product, qty = 1) {
    setCart(prev => {
      ...
      return [...prev, { productId: product.id, name: product.name, sku: product.sku || '', unitPrice: product.sellingPrice, qty }]
    })
  ...
  function changeQty(pid, d) {
    setCart(p => p.map(i => i.productId === pid ? { ...i, qty: Math.max(1, i.qty + d) } : i))
  }

  function removeFromCart(pid) {
    setCart(p => p.filter(i => i.productId !== pid))
  }
  ...
  async function placeOrder() {
    ...
    try {
      const r = await post(`/store/${slug}/orders`, { ...form, items: cart, paymentMethod: 'COD' })
