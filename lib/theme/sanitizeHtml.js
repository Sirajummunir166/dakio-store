...
// ── Rich product content (description, short description, product-page tabs) ──
// Written in the merchant app's Tiptap editor and already sanitised by the API
// (dakio-api/src/lib/richHtml.js, same allowlist); this is the render-time
// gate for the same shape: tables, https images, rules, code, colour spans.
const RICH_TAGS = new Set([
  'p', 'br', 'b', 'strong', 'i', 'em', 'u', 's', 'a', 'span',
  'h2', 'h3', 'h4', 'ul', 'ol', 'li', 'blockquote', 'hr', 'code', 'pre',
...
const RICH_STYLE_PROP = /^(color|text-align)$/i

function sanitizeRichAttrs(tag, attrStr) {
  const allowed = new Set(RICH_ATTRS[tag] || [])
  const out = []
  let hasSrc = false
  const re = /([a-zA-Z_:][-a-zA-Z0-9_:.]*)\s*(?:=\s*("([^"]*)"|'([^']*)'|[^\s"'>]+))?/g
  let m
  while ((m = re.exec(attrStr))) {
    ...
    let val = m[3] !== undefined ? m[3] : m[4] !== undefined ? m[4] : (m[2] || '')
    if (name === 'style') {
      val = sanitizeStyleAttr(val).split('; ').filter((d) => RICH_STYLE_PROP.test(d.split(':')[0])).join('; ')
      if (!val) continue
    }
    if (name === 'href') { const h = sanitizeHref(val); if (!h) continue; val = h }
    if (name === 'src') { if (!/^https:\/\//i.test(val.trim())) continue; val = val.trim(); hasSrc = true }
    if (name === 'target') val = '_blank'
    if (name === 'rel') val = 'noopener noreferrer'
    if (['colspan', 'rowspan', 'width', 'height'].includes(name) && !/^\d{1,4}$/.test(val)) continue
    out.push(name + '="' + val.replace(/"/g, '&quot;') + '"')
  }
  if (tag === 'img' && !hasSrc) return null
  return out.length ? ' ' + out.join(' ') : ''
}
...
export function sanitizeRichHtml(input) {
  ...
  html = html.replace(/<\/?([a-zA-Z][a-zA-Z0-9]*)((?:\s+[^<>]*?)?)\s*(\/?)>/g, (full, name, attrStr) => {
    ...
    if (full.startsWith('</')) return tag === 'img' || tag === 'br' || tag === 'hr' ? '' : '</' + tag + '>'
    const attrs = sanitizeRichAttrs(tag, attrStr || '')
...
/** Prose without tags — SEO descriptions, teasers, search. */
...
/** True when the HTML shows nothing (no text, image, table or rule). */
