...
// /cart — store system page (Phase 10). Themed by tokens, guarded layout.
...
export default async function CartRoute({ params }) {
  const { slug } = await params
  const siteData = await getPublishedSite(slug)
  ...
  return (
    <PublicSite storeSlug={slug} doc={siteData.site} pageId="home" basePath={'/' + slug} products={catalog.products} collections={catalog.collections} system={{ kind: 'cart' }} />
  )
}
