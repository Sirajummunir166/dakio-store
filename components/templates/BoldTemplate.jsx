...
export default function BoldTemplate(props) {
  ...
  return (
    <div style={{ fontFamily: FONT, background: '#111', minHeight: '100vh', color: '#fff' }}>
      ...
      {/* Hero */}
      {!isFiltered && (
        <div style={{ position: 'relative', width: '100%', minHeight: 'clamp(220px, 50vw, 680px)', overflow: 'hidden', background: '#000' }}>
          ...
          <div style={{ position: 'relative', zIndex: 2, maxWidth: '1280px', margin: '0 auto', padding: '80px 48px', minHeight: 'clamp(220px, 50vw, 680px)', display: 'flex', alignItems: 'center' }}>
            <div>
...
function BoldCard({ p, accent, currency, onAdd, onView, onQuick }) {
  ...
  return (
    <div onMouseEnter={() => setHovered(true)} onMouseLeave={() => setHovered(false)}
      style={{ position: 'relative', overflow: 'hidden', cursor: 'pointer', background: '#1a1a1a', aspectRatio: '3/4', display: 'flex', flexDirection: 'column' }}>
      <div style={{ position: 'relative', flex: 1 }} onClick={() => onView(p)}>
        ...
        {discount && !oos && <div style={{ position: 'absolute', top: '12px', left: '12px', background: '#dc2626', color: '#fff', fontSize: '10px', fontWeight: 900, padding: '4px 10px' }}>{discount}% off</div>}
        <div style={{ position: 'absolute', bottom: 0, left: 0, right: 0, padding: '16px' }}>
          ...
          <div style={{ fontSize: '14px', fontWeight: 900, color: '#fff', lineHeight: 1.2, marginBottom: '6px', textTransform: 'uppercase', overflow: 'hidden', display: '-webkit-box', WebkitLineClamp: 2, WebkitBoxOrient: 'vertical' }}>{p.name}</div>
          <div style={{ display: 'flex', alignItems: 'baseline', gap: '6px', marginBottom: '4px' }}>
            <div style={{ fontSize: '16px', fontWeight: 900, color: accent }}>{fmt(p.sellingPrice, currency)}</div>
            {discount && <div style={{ fontSize: '11px', color: '#9ca3af', textDecoration: 'line-through' }}>{fmt(p.mrp, currency)}</div>}
          </div>
          <div style={{ fontSize: '10px', color: '#86efac', fontWeight: 600 }}>✓ ক্যাশ অন ডেলিভারি</div>
        </div>
      </div>
      {!oos && (
        <button onClick={e => { e.stopPropagation(); onAdd(p, 1) }}
          style={{ width: '100%', padding: '10px 0', background: accent, color: '#111', border: 'none', fontSize: '11px', fontWeight: 900, cursor: 'pointer', letterSpacing: '0.08em', textTransform: 'uppercase' }}>
          অর্ডার করুন
        </button>
      )}
    </div>
