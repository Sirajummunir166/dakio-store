...
import { BadgeChip, Cta2Btn } from '../blocks';
...
export default function Story({ sec, ctx }) {
  ...
  const { c, pad, h2, btnGhost, sh } = base;

  const storyGrid = 'display:grid; grid-template-columns:' + (mob ? '1fr' : '1fr 1fr') + '; gap:' + (mob ? 26 : 60) + 'px; align-items:center;';
  const storyImgWrap = 'aspect-ratio:4/3.4; border-radius:' + C.r + 'px; overflow:hidden; position:relative; background:' + c.card + ';' + sh + (sec.v === 'right' && !mob ? ' order:2;' : '');
